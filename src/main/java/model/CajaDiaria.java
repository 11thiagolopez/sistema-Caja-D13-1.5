...
public class CajaDiaria implements Serializable {
	...
	public boolean realizarRetiroEfectivo(double monto, String motivo) {
		...
		if (monto > 0 && monto <= efectivoDisponible) {
			this.transacciones
					.add(new Transaccion(Transaccion.Tipo.RETIRO, Transaccion.MedioDePago.EFECTIVO, monto, motivo));
			return true;
		}
		return false;
	}
	...
	public boolean realizarRetiroTransferencia(double monto, String motivo) {
		...
		if (monto > 0 && monto <= transferenciaDisponible) {
			this.transacciones.add(
					new Transaccion(Transaccion.Tipo.RETIRO, Transaccion.MedioDePago.TRANSFERENCIA, monto, motivo));
			return true;
		}
		return false;
	}
	...
	public double getVentasEfectivo() {
		return transacciones.stream().filter(
				t -> t.getTipo() == Transaccion.Tipo.VENTA && t.getMedioDePago() == Transaccion.MedioDePago.EFECTIVO)
	...
	public double getVentasTransferencia() {
		return transacciones.stream()
				.filter(t -> t.getTipo() == Transaccion.Tipo.VENTA
						&& t.getMedioDePago() == Transaccion.MedioDePago.TRANSFERENCIA)
				.mapToDouble(Transaccion::getMonto).sum();
	}

	public double getVentasTarjeta() {
		return transacciones.stream().filter(
				t -> t.getTipo() == Transaccion.Tipo.VENTA && t.getMedioDePago() == Transaccion.MedioDePago.TARJETA)
				.mapToDouble(Transaccion::getMonto).sum();
	}

	public double getRetirosEfectivo() {
		return transacciones.stream().filter(
				t -> t.getTipo() == Transaccion.Tipo.RETIRO && t.getMedioDePago() == Transaccion.MedioDePago.EFECTIVO)
				.mapToDouble(Transaccion::getMonto).sum();
	}

	public double getRetirosTransferencia() {
		return transacciones.stream()
				.filter(t -> t.getTipo() == Transaccion.Tipo.RETIRO
						&& t.getMedioDePago() == Transaccion.MedioDePago.TRANSFERENCIA)
				.mapToDouble(Transaccion::getMonto).sum();
	}

	// Genera la lista de Strings para los reportes a partir de la lista de
	// transacciones.
	public List<String> getDetalleVentas() {
		return transacciones
				.stream().filter(t -> t.getTipo() == Transaccion.Tipo.VENTA).map(t -> "Producto: "
						+ t.getNombreProducto() + " - Precio: $" + t.getMonto() + " - Medio: " + t.getMedioDePago())
	...
	private Transaccion.MedioDePago convertirMedioDePago(int medioDePagoInt) {
		...
		case 1:
			return Transaccion.MedioDePago.EFECTIVO;
		case 2:
			return Transaccion.MedioDePago.TRANSFERENCIA;
		case 3:
			return Transaccion.MedioDePago.TARJETA;
		default:
			throw new IllegalArgumentException("Medio de pago inválido: " + medioDePagoInt);
		}
	}
}
